...
def template_creator():
    ...
    name = st.text_input("Enter Pkg/Func/Proc Name")
    ...
    proc = """
    CREATE OR REPLACE PROCEDURE {} (input_parameter datatype)
    IS
        DECLARE YOUR VARIABLES
    BEGIN
    ...
    if(st.checkbox('Show Package') == True):
        st.text(pkg)
    elif(st.checkbox('Show Function') == True):
        st.text(func)
    elif(st.checkbox('Show Procedure') == True):
        st.text(proc)
          
            
side_menu = st.sidebar.selectbox('Select Your Program', ['Home', 'Data Replicator', 'API Tester', 'Template Creator'])
...
if(side_menu == 'Home'):
    ...
    api_test()
elif(side_menu == 'Template Creator'):
    template_creator()